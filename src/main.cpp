...
// Copyright (c) 2017-2017 The Pegasus developers
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (Params().NetworkID() == CBaseChainParams::TESTNET) {
        if (nHeight < Params().LAST_POW_BLOCK() && nHeight > 0)
            return 10000 * COIN; // 2M PEG Premine across 200 blocks
    }

    if (nHeight < Params().LAST_POW_BLOCK())
        nSubsidy = 10000 * COIN;
    else if (nHeight <= 34480)
        nSubsidy = 15 * COIN;
    else
        nSubsidy = 190 * COIN;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // PEG: We use certain sporks during IBD, so check to see if they are
