...
if [[ $HOST = "i686-w64-mingw32" ]]; then
  ARCHIVE_NAME="peg-windows-32bit.zip"
elif [[ $HOST = "x86_64-w64-mingw32" ]]; then
    ARCHIVE_NAME="peg-windows-64bit.zip"
    ...
    ARCHIVE_NAME="peg-linux-x64.tar.gz"
    ARCHIVE_CMD="tar -czf"
...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i Pegasus-network.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/pegasus-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/pegasus-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/pegasusd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/pegasusd.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/pegasus-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/pegasus-cli.exe $OUTDIR/bin/
